...
/**
 * TypeSafe System One ("Jev") client — the general-purpose semantic question
 * engine. Jev answers typed questions (noul / choice / score) with calibrated
 * probabilities; this module is model-agnostic on purpose: VirtualPC uses it
 * for confidence-gated heuristics, classification, and workflow gating, never
 * as an unconditional oracle (see docs/JEV-ENGINE.md).
 *
 * The finance-specific feature fan-out lives in src/finance and consumes this
 * same client (src/finance/jev-client.ts re-exports it).
 */
