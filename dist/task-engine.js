...
exports.getAgentScorecard = getAgentScorecard;
exports.getAllAgentScorecards = getAllAgentScorecards;
...
function defaultTaskTags(task) {
    const project = normalizeProject(task.project);
    const projectSlug = slugifyTag(project) || 'virtualpc-platform';
    const role = agent_registry_1.ROLE_MAP[task.assigned_to || ''] || task.assigned_to || 'unassigned';
...
function ensureTaskMetadata(task) {
    task.project = normalizeProject(task.project);
    task.tags = normalizeTags(task.tags, defaultTaskTags(task));
    return task;
}
const taskPools = {
    Fill: [
...
function defaultTaskPoolFor(agent) {
    const role = agent_registry_1.ROLE_MAP[agent] || agent;
    if (agent === 'Governor') {
        return [
...
function generateTask(agent) {
    ...
    if (!taskPools[agent])
        taskPools[agent] = defaultTaskPoolFor(agent);
...
let dirty = false;
function compactTaskHistory(items) {
    const keep = new Set();
    const completedSeen = new Map();
    for (let i = items.length - 1; i >= 0; i--) {
        ...
        if (count < MAX_COMPLETED_TASKS_PER_AGENT)
            keep.add(task);
...
function isPlaceholderTask(task) {
...
function compactTasksInPlace(reason) {
    const { tasks: compacted, dropped } = compactTaskHistory(tasks);
    if (dropped > 0) {
        tasks.splice(0, tasks.length, ...compacted);
        logger_1.default.info(`task-engine: compacted ${dropped} completed historical tasks (${reason}); kept ${tasks.length}`);
    }
    return dropped;
}
function saveState() {
...
function loadState() {
    try {
        ...
        for (const t of compacted.tasks) {
            ...
            const before = JSON.stringify({ project: t.project, tags: t.tags });
        ...
        if (compacted.dropped > 0) {
            logger_1.default.info(`task-engine: dropped ${compacted.dropped} old completed tasks during restore (kept last ${MAX_COMPLETED_TASKS_PER_AGENT}/agent)`);
            dirty = true;
        }
        if (migratedMetadata)
            dirty = true;
        ...
        for (const task of inProgress) {
            ...
            if (doneCount === task.subtasks.length) {
                ...
                generateArtifactForCompletedTask(agent, task).catch(err => {
                    logger_1.default.warn(`artifact gen failed for ${agent}/${task.id}: ${err.message}`);
                    ...
                    try {
                        ...
                        bestEffortPublish((p) => p.publishTaskFailed({
                            task_id: task.id,
                            agent,
                            title: task.title,
...
function gradeFromAgentScore(score) {
    if (score >= 90)
        return 'A';
    if (score >= 80)
        return 'B';
...
function getAgentScorecard(agentName) {
    const agentTasks = tasks.filter(t => t.assigned_to === agentName);
    const completed = agentTasks.filter(t => t.status === 'completed').length;
    const active = agentTasks.filter(t => t.status === 'in-progress');
    ...
    if (lastActivity) {
        const ageHours = (Date.now() - new Date(lastActivity).getTime()) / 3600000;
        if (ageHours > 72) {
            score -= 20;
    ...
    const status = blockers > 0 || placeholders > 0 || score < 70
        ? 'needs_attention'
        : active.length > 0
            ? 'working'
    ...
    return {
        agent: agentName,
        role: agent_registry_1.ROLE_MAP[agentName] || agentName,
        score,
...
function getAllAgentScorecards() {
    return agent_registry_1.AGENT_NAMES.map(getAgentScorecard);
}
...
function setTaskStatus(taskId, next) {
    ...
    if (next === 'completed' && previous !== 'completed') {
        try {
            const { bestEffortPublish } = require('./integrations/kafka/shared');
            bestEffortPublish((p) => p.publishResult(task.id, task.assigned_to, {
...
function addTask(input) {
    if (!agent_registry_1.AGENT_NAMES.includes(input.assigned_to))
        return null;
    ...
    try {
        const { bestEffortPublish } = require('./integrations/kafka/shared');
        bestEffortPublish((p) => p.publishTask(input.assigned_to, {
            task_type: 'delegated',
            priority: t.priority,
...
if (globalThis.__virtualpcPersistedWorkLog) {
    const persisted = globalThis.__virtualpcPersistedWorkLog;
    ...
    for (let i = start; i < persisted.length; i++)
        workLog.push(persisted[i]);
...
const cliSessionLog = Object.fromEntries(agent_registry_1.AGENT_NAMES.map(agent => [agent, []]));
